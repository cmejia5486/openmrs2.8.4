...
- Itera los requisitos y decide: YES / NO / N/A / MANUAL.
- Evita "YES" si no hay evidencia positiva clara; ante duda -> MANUAL.
- Preselección opcional (src-catm*) según capacidades detectadas en el código.
- Integra evidencias de SARIF (SAST), MobSF (estático/dinámico), Trivy y análisis del AndroidManifest/network_security_config.
- Muestra progreso [i/N] y porcentaje.
- Limita a N requisitos con --max-requirements.
- Usa OpenAI solo si se pide explícitamente: --use-llm {never,uncertain,always} (por defecto: never).
- Genera: audit-findings.json, secm-catm.docx (siguiendo tu formato), checklist.docx, audit-summary.docx/md.
...
def safe_json_extract(s: str) -> Dict[str, Any] | None:
    """Extrae el primer objeto JSON bien formado del texto."""
    stack = 0
    start = None
...
class CodeHints:
    ...
    present: set[str]     # hints de riesgo presentes (regex.pattern)
    caps: set[str]        # capacidades inferidas
...
def parse_network_security_config(app_root: Path, ns_path: Path) -> Dict[str, Any]:
    """Devuelve {'base_cleartext_true': bool, 'any_domain_cleartext_true': bool}."""
...
def parse_manifest(source_root: Path) -> ManifestInfo:
    ...
    if manifest and manifest.exists():
        try:
            ...
            # targetSdkVersion si está en uses-sdk
    ...
    # agregar flags del network_security_config
    if ns_cfg_path:
        ns = parse_network_security_config(source_root, ns_cfg_path)
        # Si base/domain cleartext true, y uses_cleartext_traffic aún None -> considerar riesgo
        if ns.get("base_cleartext_true") or ns.get("any_domain_cleartext_true"):
            # no fuerza uses_cleartext_traffic, pero es evidencia de configuración insegura
            pass
...
# ---- Parseo de evidencias (SARIF, MobSF, Trivy) ------------------------------
@dataclass
class Evidence:
...
# ---- Resumen de contexto para el LLM ----------------------------------------
...
# ---- PROMPT MAESTRO (largo, con few-shot) -----------------------------------
...
def llm_decide(puid: str, req: Dict[str, Any], app_ctx: str, code: CodeHints, ev: Evidence, timeout_s: int) -> Tuple[str, str, Dict[str, Any]]:
    ...
    # Soft timeout por llamada (mejor que bloquear el job)
    start = time.time()
    try:
        ...
        return ("MANUAL", f"LLM error: {e}", {"manual_steps": ["Revisar manualmente por contexto insuficiente"]})

    obj = safe_json_extract(text or "")
    if not obj or "decision" not in obj:
        return ("MANUAL", "Unparseable LLM output", {"manual_steps": ["Revisar manualmente por salida no válida"]})
...
def heuristic_decide(req: Dict[str, Any], code: CodeHints, ev: Evidence) -> Tuple[str, str, bool]:
    ...
    - NO si hay evidencia contradictoria directa (p. ej. allowBackup=true ante política de backup off;
      debuggable=true en release; cleartext permitido cuando se exige TLS; componentes exportados con IF sin control)
    - N/A si el requisito depende de una capability ausente (webview/biometric/nfc/etc.)
    - YES solo con evidencia positiva o ausencia de contradicción + hints de cumplimiento
    - MANUAL si no hay evidencia suficiente
    """
    ...
    # 1) Reglas específicas por requisito frecuente
    # TLS / Cleartext
    if _req_has(text, ["https", "tls", "cleartext", "http only forbidden", "no cleartext"]):
        ...
        # sin contradicción pero sin prueba fuerte
        return ("MANUAL", "No contradicción visible, falta corroborar en runtime", True)
    ...
    if _req_has(text, ["backup", "allowbackup"]):
        ...
        return ("MANUAL", "allowBackup not declared -> requiere revisión", True)
    ...
    if _req_has(text, ["export", "exported", "implicit exported", "component exposure"]):
        ...
        # Si hay IF sin exported explícito en target 31+, es riesgo/instalable
    ...
    if _req_has(text, ["pii", "logs", "no sensitive info in logs", "logging"]):
        if any("Log.d" in m or "Log.i" in m for m in code.matches):
            return ("MANUAL", "Necesita ver logcat en runtime", True)
        return ("MANUAL", "No evidencia estática suficiente; requiere runtime", True)

    # Capacidades ausentes -> N/A
    caps_text_pairs = [
        ("saml", "saml"),
        ("webview", "webview"),
        ("biometric", "biometric"),
    ...
    # Generales: si hay palabras fuertes de seguridad y no vemos contradicción -> MANUAL (conservador)
    if any(k in text for k in ["tls", "encrypt", "permission", "session", "token", "cookie", "audit", "pinning"]):
        if hints:
            return ("MANUAL", "Evidencia estática insuficiente para concluir", True)
        return ("MANUAL", "Sin evidencia suficiente", True)

    # Por defecto
    return ("MANUAL", "Insuficiente información para concluir", True)
...
def write_docx_summary(path: Path, totals: Dict[str, int], template: Optional[Path]) -> None:
    ...
    if not template:
        ...
        # Si hay una tabla en la plantilla, intenta llenar la primera fila vacía
        # (Dejamos el comportamiento simple/robusto; tu plantilla ya trae estructura)
        doc.add_paragraph(
            f"Total requirements: {totals['total']} | YES: {totals['YES']} | NO: {totals['NO']} | N/A: {totals['N/A']} | MANUAL: {totals['MANUAL']}"
        )
    doc.save(path.as_posix())
...
def main() -> int:
    ...
    ap.add_argument("--checklist", type=Path, required=True, help="Ruta a requirements.json")
    ap.add_argument("--reports", type=Path, required=True, help="Carpeta con artefactos (MobSF/SARIF/Trivy)")
    ap.add_argument("--source-root", type=Path, required=True, help="Raíz del repo para pistas rápidas")
    ap.add_argument("--output-dir", type=Path, required=True, help="Carpeta de salida")
    ap.add_argument("--max-requirements", type=int, default=-1, help="Limita el número de requisitos a auditar")
    ap.add_argument("--preselect", action="store_true", help="Preseleccionar src-catm* según capacidades")
    ap.add_argument("--mark-manual-when-uncertain", action="store_true", help="N/A incierto -> MANUAL")
    ap.add_argument("--use-llm", choices=["never", "uncertain", "always"], default="never",
                    help="Cuándo invocar LLM (por defecto: never)")
    ap.add_argument("--llm-timeout", type=int, default=25, help="Timeout blando por requisito (s)")
    ap.add_argument("--app-context", type=Path, default=None, help="Archivo con contexto (README/docs)")
    ap.add_argument("--catm-template", type=Path, default=None, help="Plantilla DOCX para secm-catm")
    ap.add_argument("--summary-template", type=Path, default=None, help="Plantilla DOCX para audit summary")
    ...
    for idx, r in enumerate(selected, start=1):
        ...
        # Primero heurística (conservadora)
        decision, evidence, uncertain = heuristic_decide(r, code, ev)

        # LLM según política --use-llm
        ...
        if need_llm and OPENAI_AVAILABLE and os.getenv("OPENAI_API_KEY1"):
            ...
            if llm_dec == "NO":
                ...
                # si seguíamos con duda, adoptamos salida del LLM
                decision, evidence, extras = llm_dec, llm_ev, llm_ex
            else:
                extras = {}
